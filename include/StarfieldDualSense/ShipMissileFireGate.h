...
namespace sds
{
    inline constexpr std::uint32_t kHardwareObservedShipMissileFireEventId = 0x6846C9ECu;

    [[nodiscard]] constexpr bool isHardwareObservedShipMissileFireEvent(
        std::uint32_t eventId) noexcept
    {
        return eventId == kHardwareObservedShipMissileFireEventId;
    }

    struct ShipMissileDeferredFire
    {
    ...
    private:
        ...
        [[nodiscard]] ShipMissileDeferredFire consumePendingFirstShot(
            std::chrono::steady_clock::time_point when) noexcept;
