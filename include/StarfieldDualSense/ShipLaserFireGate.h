...
namespace sds
{
    inline constexpr std::uint32_t kHardwareObservedShipPulseLaserFireEventId = 0xCE7B2EB1u;

    [[nodiscard]] constexpr bool isHardwareObservedShipPulseLaserFireEvent(
        std::uint32_t eventId) noexcept
    {
        return eventId == kHardwareObservedShipPulseLaserFireEventId;
    }

    struct ShipLaserDeferredFire
    {
    ...
    struct ShipLaserTriggerObservation
    {
        ShipLaserDeferredFire deferredFire{};
        bool stopped{ false };
    };

    class ShipLaserFireGate
    {
    public:
        void setPilotActive(bool active) noexcept;
        void setMenuBlocked(bool blocked) noexcept;
        ShipLaserTriggerObservation observeRightTrigger(
            std::uint8_t r2,
    ...
    private:
        ...
        [[nodiscard]] ShipLaserDeferredFire consumePendingFirstShot(
            std::chrono::steady_clock::time_point when) noexcept;
