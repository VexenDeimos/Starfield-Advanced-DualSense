...
namespace sds
{
    inline constexpr std::uint32_t kHardwareObservedShipEMFireEventId = 0x7A1A570Cu;

    [[nodiscard]] constexpr bool isHardwareObservedShipEMFireEvent(
        std::uint32_t eventId) noexcept
    {
        return eventId == kHardwareObservedShipEMFireEventId;
    }

    struct ShipEMDeferredFire
    {
    ...
    private:
        ...
        [[nodiscard]] ShipEMDeferredFire consumePendingFirstShot(
            std::chrono::steady_clock::time_point when) noexcept;
