...
void sds::ShipLaserFireGate::clearPendingFirstShot() const noexcept
{
...
void sds::ShipLaserFireGate::clearStreamAuthority() noexcept
{
...
void sds::ShipLaserFireGate::rememberPendingFirstShot(
...
sds::ShipLaserDeferredFire sds::ShipLaserFireGate::consumePendingFirstShot(
    std::chrono::steady_clock::time_point when) noexcept
{
    ShipLaserDeferredFire result{};
    while (pendingFirstShotLock_.test_and_set(std::memory_order_acquire)) {
    }
    ...
    result.eventId = kHardwareObservedShipPulseLaserFireEventId;
...
void sds::ShipLaserFireGate::setPilotActive(bool active) noexcept
{
...
void sds::ShipLaserFireGate::setMenuBlocked(bool blocked) noexcept
{
...
sds::ShipLaserTriggerObservation sds::ShipLaserFireGate::observeRightTrigger(
    std::uint8_t r2,
    std::chrono::steady_clock::time_point when) noexcept
{
    ShipLaserTriggerObservation result{};
    if (!pilotActive_.load(std::memory_order_acquire) ||
        menuBlocked_.load(std::memory_order_acquire)) {
        return result;
    }
    ...
    if (r2 < 24u) {
        const bool wasAuthorized = automaticStreamArmed_.load(std::memory_order_acquire);
        clearStreamAuthority();
        if (previousR2 >= 24u) {
            clearPendingFirstShot();
            result.stopped = wasAuthorized;
        }
        return result;
    }

    lastPressedMicros_.store(micros(when), std::memory_order_release);
    result.deferredFire = consumePendingFirstShot(when);
    return result;
}

bool sds::ShipLaserFireGate::authorizeWwiseFire(
    ...
    std::chrono::steady_clock::time_point when) const noexcept
{
    if (!pilotActive_.load(std::memory_order_acquire) ||
        menuBlocked_.load(std::memory_order_acquire) ||
        !isHardwareObservedShipPulseLaserFireEvent(eventId)) {
        return false;
    }
    ...
    // Unlike the ballistic path, the hardware-proven pulse-laser heartbeat
    // precedes the physical R2 proof on the first pulse. Once R2 is released,
    // a late Wwise tail must never directly resurrect laser feedback. Remember
    // it only as a bounded Wwise-first candidate; a fresh real R2 sample is
