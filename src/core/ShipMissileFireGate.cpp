...
void sds::ShipMissileFireGate::clearPendingFirstShot() const noexcept
{
...
void sds::ShipMissileFireGate::clearStreamAuthority() noexcept
{
...
void sds::ShipMissileFireGate::rememberPendingFirstShot(
...
sds::ShipMissileDeferredFire sds::ShipMissileFireGate::consumePendingFirstShot(
    std::chrono::steady_clock::time_point when) noexcept
{
    ShipMissileDeferredFire result{};
    while (pendingFirstShotLock_.test_and_set(std::memory_order_acquire)) {
    }
    ...
    result.eventId = kHardwareObservedShipMissileFireEventId;
...
void sds::ShipMissileFireGate::setPilotActive(bool active) noexcept
{
...
void sds::ShipMissileFireGate::setMenuBlocked(bool blocked) noexcept
{
...
sds::ShipMissileDeferredFire sds::ShipMissileFireGate::observeRightTrigger(
    std::uint8_t r2,
    std::chrono::steady_clock::time_point when) noexcept
{
    if (!pilotActive_.load(std::memory_order_acquire) ||
...
bool sds::ShipMissileFireGate::authorizeWwiseFire(
    ...
    std::chrono::steady_clock::time_point when) const noexcept
{
    if (!pilotActive_.load(std::memory_order_acquire) ||
        menuBlocked_.load(std::memory_order_acquire) ||
        !isHardwareObservedShipMissileFireEvent(eventId)) {
        return false;
    }
    ...
    // Hardware showed the missile launch event about 75-107 ms before physical
    // R2 proof on fresh pulls. Wwise alone stays inert: remember only the exact
    // launcher object long enough for a real R2 sample to release it.
    if (currentR2 < 24u) {
