...
void sds::ShipEMFireGate::clearPendingFirstShot() const noexcept
{
...
void sds::ShipEMFireGate::clearStreamAuthority() noexcept
{
...
void sds::ShipEMFireGate::rememberPendingFirstShot(
...
sds::ShipEMDeferredFire sds::ShipEMFireGate::consumePendingFirstShot(
    std::chrono::steady_clock::time_point when) noexcept
{
    ShipEMDeferredFire result{};
    while (pendingFirstShotLock_.test_and_set(std::memory_order_acquire)) {
    }
    ...
    result.eventId = kHardwareObservedShipEMFireEventId;
...
void sds::ShipEMFireGate::setPilotActive(bool active) noexcept
{
...
void sds::ShipEMFireGate::setMenuBlocked(bool blocked) noexcept
{
...
sds::ShipEMDeferredFire sds::ShipEMFireGate::observeRightTrigger(
    std::uint8_t r2,
    std::chrono::steady_clock::time_point when) noexcept
{
    if (!pilotActive_.load(std::memory_order_acquire) ||
...
bool sds::ShipEMFireGate::authorizeWwiseFire(
    ...
    std::chrono::steady_clock::time_point when) const noexcept
{
    if (!pilotActive_.load(std::memory_order_acquire) ||
        menuBlocked_.load(std::memory_order_acquire) ||
        !isHardwareObservedShipEMFireEvent(eventId)) {
        return false;
    }
    ...
    // Hardware showed the EM discharge event about 81-99 ms before physical
    // R2 proof on fresh pulls. Wwise alone stays inert: remember only the exact
    // weapon-scoped object long enough for a real R2 sample to release it.
    if (currentR2 < 24u) {
