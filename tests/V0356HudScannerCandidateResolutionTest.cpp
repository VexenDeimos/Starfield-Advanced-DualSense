...
int main()
{
    constexpr std::array<std::uint32_t, 3> expectedIds{{
        0x12D8B183u,
        0x1F770B61u,
        0x06D80D5Eu,
    }};

    const auto targets = sds::uiAudioResolutionTargets();
    require(targets.size() == expectedIds.size(), "runtime catalog contains exactly three focused HUD/scanner candidates");
    std::set<std::uint32_t> unique;
    for (std::size_t i = 0; i < targets.size(); ++i) {
        require(targets[i].eventId == expectedIds[i], "runtime catalog preserves approved focused evidence order");
        require(!targets[i].label.empty(), "each focused HUD/scanner candidate has a stable nonempty label");
        unique.insert(targets[i].eventId);
    }
    require(unique.size() == targets.size(), "focused HUD/scanner catalog contains no duplicate event ids");

    const auto root = std::filesystem::temp_directory_path() / "sds_v0356_hud_scanner_candidate_resolution";
    ...
    const std::string json =
        ...
        "{\"Id\":\"" + std::to_string(eventId) + "\",\"Name\":\"Play_UI_HudScanner\",\"ReferencedStreamedFiles\":[{\"Id\":\"910001\"}]}]}]}}";
    ...
    const auto resolved = resolver.resolveObservedUiEvent("scanner-12D8B183", eventId, "v0.3.56");
    require(resolved.found, "focused HUD/scanner event resolves through existing Wwise metadata");
    require(resolved.eventName == "Play_UI_HudScanner", "HUD/scanner event resolution retains the Wwise event name");
    require(resolved.media.size() == 1u && resolved.media.front().mediaId == mediaId,
        "HUD/scanner event resolution returns its referenced WEM media");
    require(resolved.media.front().capture.status == "written",
        "resolved HUD/scanner WEM is extracted for listening");

    const auto captureRoot = data / "SFSE" / "Plugins" / "StarfieldDualSenseDiagnostics" /
        "v0.3.56" / "UiWemCandidates";
    const auto expectedFile = captureRoot / "scanner-12D8B183" / "12D8B183" / "910001.wem";
    const auto manifest = captureRoot / "manifest.tsv";
    require(std::filesystem::exists(expectedFile), "HUD/scanner WEM uses the bounded v0.3.56 diagnostic path");
    require(std::filesystem::exists(manifest), "HUD/scanner extraction writes a manifest");
    require(lineCount(manifest) == 2u, "HUD/scanner manifest contains one header and one resolved media row");

    const auto duplicate = resolver.resolveObservedUiEvent("scanner-12D8B183", eventId, "v0.3.56");
    require(duplicate.media.size() == 1u && duplicate.media.front().capture.status == "exists-same",
        "re-resolving the same HUD/scanner candidate deduplicates identical WEM bytes");
    require(lineCount(manifest) == 2u, "HUD/scanner manifest deduplicates repeated resolution rows");

    const auto missing = resolver.resolveObservedUiEvent("missing", 0xDEADBEEFu, "v0.3.56");
    require(!missing.found && missing.media.empty(), "missing HUD/scanner event fails soft without extraction");

    std::filesystem::remove_all(root);
    std::cout << "PASS v0.3.56 focused HUD/scanner candidate resolution and WEM extraction\n";
    return 0;
}
